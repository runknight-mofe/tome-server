...
--
-- Fields NOT accepted from the application layer (DB owns them):
--   id          — generated by the DB via gen_random_uuid(); any supplied
--                 value is silently discarded.
--   created_at  — set to CURRENT_TIMESTAMP at INSERT; never from input.
--   modified_at — set to CURRENT_TIMESTAMP at INSERT; updated automatically
--                 on every UPDATE by the trg_predicates_set_modified_at trigger.
--
...
BEGIN
    FOREACH v_row IN ARRAY p_rows LOOP
        ...
        -- If the caller supplies an 'id' that already exists in the DB, the
        -- predicate was previously persisted (e.g. fetched then re-submitted).
        ...
        IF v_pid IS NULL THEN
            -- New predicate: id, created_at, modified_at are owned by the DB.
...
BEGIN
    FOREACH v_row IN ARRAY p_rows LOOP
        ...
        -- modified_at is not in the SET clause: the trg_set_modified_at trigger
        -- sets it to CURRENT_TIMESTAMP automatically on every UPDATE row.
        UPDATE predicates
